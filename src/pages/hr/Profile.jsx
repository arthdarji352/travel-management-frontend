...
const Profile = () => {
  ...
  const handleSubmit = async e => {
    ...
    if (newPassword !== confirmPassword) {
      return Swal.fire({ icon: 'error', title: 'Mismatch!', text: 'Passwords do not match.', background: '#0d1117', color: '#fff', confirmButtonColor: '#2563eb' });
    }
    if (newPassword.length < 6) {
      return Swal.fire({ icon: 'error', title: 'Too Short!', text: 'Min 6 characters.', background: '#0d1117', color: '#fff', confirmButtonColor: '#2563eb' });
    }
    setSaving(true);
    try {
    ...
    <div className="p-6 bg-gray-950 min-h-screen">
      <div className="max-w-lg space-y-6">
        ...
        <div className="bg-[#0d1117] border border-white/5 rounded-2xl p-6 flex items-center gap-5">
          <div className="w-16 h-16 rounded-2xl bg-green-600 flex items-center justify-center text-white text-2xl font-bold shadow-lg shadow-green-500/20 flex-shrink-0">
            {user?.name?.charAt(0).toUpperCase()}
          </div>
          <div>
            <p className="text-white text-lg font-bold">{user?.name}</p>
            <p className="text-gray-400 text-sm">{user?.email}</p>
            <span className="mt-2 inline-block px-2.5 py-0.5 bg-green-500/10 text-green-400 text-xs rounded-full capitalize border border-green-500/20">{user?.role}</span>
          </div>
        </div>
