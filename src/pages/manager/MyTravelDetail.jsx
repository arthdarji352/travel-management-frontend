...
export default function TravelDetail() {
  ...
  const completeTrip = async () => {
    ...
    if (!r.isConfirmed) return;
    try { await API.put(`/travel/${id}/complete`); SA.success('Trip Completed! 🎉',''); navigate('/manager/my-travels',{replace:true}); }
  ...
  const cancelTravel = async () => {
    ...
    if (!r.isConfirmed) return;
    try { await API.put(`/travel/${id}/cancel`); SA.success('Cancelled',''); navigate('/manager/my-travels',{replace:true}); }
    catch (err) { SA.error('Failed', err.response?.data?.message); }
  };
