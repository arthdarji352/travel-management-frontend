...
import { StatusBadge, SerialNo, LoadingScreen, EmptyState, fmtDate, fmtMoney, SA, SearchInput } from '../../components/employee/shared.jsx';
...
const fi = e => { e.target.style.borderColor='rgba(99,102,241,0.6)'; };
...
const NewTravelModal = ({ onClose, onSuccess }) => {
  ...
  const submit = async (e) => {
    ...
    if (toDateRef.current.value < fromDateRef.current.value) return SA.error('Invalid Dates','Return date must be after departure.');
    setSaving(true);
    try {
      ...
      if (wantsAdvance && advAmtRef.current?.value) {
        try {
          await API.post('/advance', { amount: Number(advAmtRef.current.value), reason: advReasonRef.current?.value?.trim() || 'Advance required for travel', travelRequest: travel._id });
          advanceSent = true;
        } catch (_) {}
      }
        ...
        <form onSubmit={submit} style={{ padding:'24px', display:'flex', flexDirection:'column', gap:'16px' }}>
          ...
          <div>
            ...
            <p style={{ color:'#4b5563', fontSize:'11px', marginTop:'6px' }}>
              {bookingType==='hr' ? 'After manager approval, HR team will book your tickets.' : 'You arrange own tickets. No HR action needed.'}
            </p>
          </div>
          ...
          <div style={{ borderTop:'1px solid rgba(255,255,255,0.05)', paddingTop:'16px' }}>
            <button type="button" onClick={() => setWantsAdvance(v=>!v)}
              ...
              <div style={{ width:'36px', height:'20px', borderRadius:'10px', background:wantsAdvance?'#eab308':'#374151', display:'flex', alignItems:'center', padding:'2px', transition:'background 0.2s', flexShrink:0 }}>
              ...
              <span style={{ color:'#d1d5db', fontSize:'14px', display:'flex', alignItems:'center', gap:'6px' }}>
                <Wallet size={15} color="#eab308" style={{ display:'inline' }}/> Also request advance money
              </span>
            </button>
            {wantsAdvance && (
              <div style={{ marginTop:'12px', padding:'16px', background:'rgba(234,179,8,0.05)', border:'1px solid rgba(234,179,8,0.15)', borderRadius:'12px', display:'flex', flexDirection:'column', gap:'12px' }}>
                <div><label style={LS.label}>Amount Required (₹)</label><input ref={advAmtRef} type="number" min="1" style={LS.input} placeholder="5000" onFocus={fi} onBlur={fo}/></div>
          ...
          <button type="submit" disabled={saving}
            style={{ width:'100%', padding:'12px', borderRadius:'12px', border:'none', background:saving?'#312e81':'#4f46e5', color:'#fff', fontWeight:600, fontSize:'14px', cursor:saving?'not-allowed':'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'8px' }}>
...
// ── Main ──────────────────────────────────────────────────────────────────────
export default function TravelManage() {
  ...
  return (
    <div className="p-6 space-y-6 bg-gray-950 min-h-screen">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-white text-2xl font-bold">Travel Manage</h1>
          <p className="text-gray-500 text-sm mt-0.5">{travels.length} total request{travels.length!==1?'s':''}</p>
        </div>
        <button onClick={() => setShowNew(true)}
          className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition shadow-lg shadow-indigo-500/20">
          <Plus size={17}/> New Travel Request
        </button>
      </div>
        ...
        {STATUS_TABS.filter(s => counts[s] > 0 || s==='all').map(s => (
          <button key={s} onClick={() => setTab(s)}
            className={`px-3 py-1.5 rounded-full text-xs font-medium capitalize transition ${
              tab===s ? 'bg-indigo-600 text-white' : 'bg-gray-900 text-gray-400 hover:text-white border border-white/5'
            }`}>
            {s} ({counts[s]})
          </button>
        ))}
      ...
      {filtered.length === 0
        ? <EmptyState
            message={travels.length===0 ? "No travel requests yet." : "No trips match your filter."}
            onAction={travels.length===0 ? () => setShowNew(true) : undefined}
                ...
                {filtered.map((t,i) => (
                  <tr key={t._id} onClick={() => navigate(`/employee/travels/${t._id}`)}
                    ...
                    <td className="py-3 px-4 text-gray-500 text-xs whitespace-nowrap">
                      {fmtDate(t.fromDate)}<br/><span className="text-gray-700">{fmtDate(t.toDate)}</span>
                    </td>
      ...
      {showNew && (
        <NewTravelModal
          ...
          onSuccess={(adv) => {
            ...
            SA.success('✈️ Submitted!', adv ? 'Travel + advance sent to manager.' : 'Travel request sent to manager.');
          }}
        />
      )}
    </div>
